...
CODE_EXTENSIONS = {
    "txt", "md", "json", "csv", "log", "sql", "ts", "tsx", "js", "jsx",
    "py", "html", "css", "xml", "yml", "yaml", "sh", "env"
}


def detect_category(file_name: str, mime_type: str | None = None) -> tuple[str, AssetCategory]:
    ...
    if ext in DOCUMENT_EXTENSIONS or "pdf" in mime or "officedocument" in mime or "msword" in mime:
        return ext, AssetCategory.DOCUMENT
    if ext in ARCHIVE_EXTENSIONS or "zip" in mime or "compressed" in mime or "tar" in mime:
    ...
    if ext in CODE_EXTENSIONS or mime.startswith("text/") or "json" in mime or "csv" in mime:
