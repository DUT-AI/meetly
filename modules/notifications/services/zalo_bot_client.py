...
class ZaloBotClient:
    ...
    async def send_message(
        ...
        try:
            ...
            async with httpx.AsyncClient() as hc:
                ...
                logger.debug(
                    f"Đã gửi tin nhắn Zalo Bot tới {chat_id}. Response: {res.text}"
                )
                return res.json()
    ...
    async def send_photo(
        ...
        try:
            ...
            payload = {"chat_id": chat_id, "caption": caption, "photo": photo}
            async with httpx.AsyncClient() as hc:
                res = await hc.post(url, json=payload)
                res.raise_for_status()
    ...
    async def send_sticker(self, chat_id: str, sticker: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            async with httpx.AsyncClient() as hc:
                ...
                res.raise_for_status()
                logger.debug(
                    f"Đã gửi sticker Zalo Bot tới {chat_id}. Response: {res.text}"
                )
                return res.json()
        except Exception as e:
