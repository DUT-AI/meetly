...
def create_mock_binary_frame(
    stream_id: int, flags: int, seq: int, start_sample: int, pcm_data: np.ndarray
) -> bytes:
...
async def test_stream_ingestion_full_lifecycle():
    ...
    # Frame 0: Speech
    frame0 = create_mock_binary_frame(
        stream_id=1, flags=0, seq=0, start_sample=0, pcm_data=pcm_speech
    )
    # Frame 1..8: Silence to trigger endpointing
    silence_frames = [
        create_mock_binary_frame(
            stream_id=1, flags=0, seq=i, start_sample=i * 1600, pcm_data=pcm_silence
        )
        for i in range(1, 9)
    ]
    # Frame 9: EOS frame
    eos_frame = create_mock_binary_frame(
        stream_id=1,
        flags=1,
        seq=9,
        start_sample=9 * 1600,
    ...
    mock_ws.receive.side_effect = [
        {"type": "websocket.receive", "bytes": f} for f in raw_frames
    ]

    with (
        patch(
            "modules.transcription.use_cases.stream_ingestion_use_case.SqlTranscriptSegmentRepository"
        ) as mock_segment_repo_cls,
        patch(
            "modules.transcription.use_cases.stream_ingestion_use_case.SqlTranscriptionSessionRepository"
        ) as mock_session_repo_cls,
        patch(
            "modules.transcription.use_cases.stream_ingestion_use_case.SileroVADDetector"
        ) as mock_vad_cls,
    ):
        mock_segment_repo = mock_segment_repo_cls.return_value
        mock_segment_repo.upsert_segment = AsyncMock(
            return_value=MagicMock(id="seg_saved123")
        )
...
async def test_stream_ingestion_speaker_identification_and_dual_stream():
    ...
    ctrl_msg = {
        ...
        "text": '{"type":"speaker_update","stream_id":1,"speaker_name":"Phước Nguyễn"}',
    }
    ...
    mock_ws.receive.side_effect = (
        [ctrl_msg, {"type": "websocket.receive", "bytes": frame0}]
    ...
    with (
        ...
        mock_segment_repo = mock_segment_repo_cls.return_value
        mock_segment_repo.upsert_segment = AsyncMock(
            return_value=MagicMock(id="seg_saved456")
        )
