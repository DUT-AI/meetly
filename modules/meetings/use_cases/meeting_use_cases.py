...
async def _send_meeting_discord_zalo_notification(
    ...
    try:
        ...
        local_start = start_time.astimezone(vn_tz) if start_time.tzinfo else start_time.replace(tzinfo=timezone.utc).astimezone(vn_tz)
...
class MeetingUseCases:
    ...
    async def _check_admin(self, workspace_id: str, user_id: str) -> None:
        ...
        if not member or member.role != MemberRole.ADMIN:
            raise HTTPException(
                status_code=403, detail="Chỉ có Admin của Workspace mới có quyền thực hiện thao tác này."
            )
    ...
    async def create_meeting(
        ...
        st = start_time.astimezone(timezone.utc) if start_time.tzinfo else start_time.replace(tzinfo=timezone.utc)
        et = end_time.astimezone(timezone.utc) if end_time.tzinfo else end_time.replace(tzinfo=timezone.utc)

        if st < now - timedelta(seconds=60):
            raise HTTPException(status_code=400, detail="Thời gian bắt đầu không được ở quá khứ")
        if et <= st:
            raise HTTPException(status_code=400, detail="Thời gian kết thúc phải lớn hơn thời gian bắt đầu")
        if not participants:
            raise HTTPException(status_code=400, detail="Cuộc họp phải có ít nhất 1 người tham gia")
    ...
    async def list_meetings(self, workspace_id: str, actor_id: str) -> list[MeetingEntity]:
    ...
    async def update_meeting(
        ...
        new_participant_ids = set(str(p) for p in (participants or [])) if participants is not None else old_participant_ids
        ...
        if newly_added_ids:
            ...
            vn_tz = timezone(timedelta(hours=7))
            local_start = effective_start.astimezone(vn_tz) if effective_start.tzinfo else effective_start.replace(tzinfo=timezone.utc).astimezone(vn_tz)
            time_str = local_start.strftime("%d/%m/%Y %H:%M")
            ...
            logger.info(f"Sent meeting_participant_added notification to {len(newly_added_ids)} new participant(s).")
        ...
        if title_changed or time_changed:
            ...
            logger.info(f"Sent meeting_updated notification to {len(all_participant_ids)} participant(s) due to: {change_description}.")
    ...
    async def delete_meeting(self, meeting_id: str, actor_id: str) -> None:
        ...
        logger.info(f"Sent meeting_deleted notification to {len(meeting.participants or [])} participant(s).")
