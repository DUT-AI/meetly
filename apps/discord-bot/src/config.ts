...
const configSchema = z.object({
  ...
  MINIO_BUCKET: z
    .string()
    .default(
      process.env.DEFAULT_BUCKET || process.env.MINIO_BUCKET || "meetly-dev",
    ),
  MINIO_USE_SSL: z
    .boolean()
    .default(
  ...
  TEMP_DIR: z
    .string()
    .default(process.env.TEMP_DIR || path.resolve(process.cwd(), "tmp")),
  LOG_LEVEL: z
    .enum(["debug", "info", "warn", "error"])
    .default((process.env.LOG_LEVEL as any) || "info"),
  NODE_ENV: z
    .enum(["development", "production", "test"])
    .default((process.env.NODE_ENV as any) || "development"),
  ENABLE_PREVIEW_URL: z
    .boolean()
    .default(process.env.ENABLE_PREVIEW_URL === "true"),
...
const parsed = configSchema.safeParse({
  ...
  PREVIEW_URL_EXPIRES_IN_SECONDS:
    process.env.PREVIEW_URL_EXPIRES_IN_SECONDS || "900",
});

if (!parsed.success) {
  console.warn(
    "Environment warning: Some required keys may be missing (e.g. DISCORD_BOT_TOKEN).",
  );
...
export const config = parsed.success
  ...
  : ({
      ...
      MINIO_ENDPOINT: normalizedEndpoint,
      MINIO_ACCESS_KEY:
        process.env.MINIO_ACCESS_KEY || process.env.MINIO_ROOT_USER || "dutai",
      MINIO_SECRET_KEY:
        process.env.MINIO_SECRET_KEY ||
        process.env.MINIO_ROOT_PASSWORD ||
        "dutai123",
      ...
      MINIO_USE_SSL:
        ...
        process.env.MINIO_SECURE === "true" ||
        process.env.MINIO_USE_SSL === "true",
      MAX_MEETING_DURATION_MINUTES: parseInt(
        process.env.MAX_MEETING_DURATION_MINUTES || "180",
        10,
      ),
      SILENCE_TIMEOUT_MINUTES: parseInt(
        process.env.SILENCE_TIMEOUT_MINUTES || "5",
        10,
      ),
      ...
      ENABLE_PREVIEW_URL: process.env.ENABLE_PREVIEW_URL === "true",
      PREVIEW_URL_EXPIRES_IN_SECONDS: parseInt(
        process.env.PREVIEW_URL_EXPIRES_IN_SECONDS || "900",
        10,
      ),
