...
from .engine import SeamlessTranslationEngine, WhisperASREngine

class TranslateRequest(BaseModel):
    text: str = Field(..., description="Text to translate")
    src_lang: str = Field(default="vie", description="Source language code (NLLB/Seamless 3-letter: vie)")
    tgt_lang: str = Field(default="eng", description="Target language code (NLLB/Seamless 3-letter: eng)")
...
class TranscribeRequest(BaseModel):
    audio_base64: str = Field(..., description="Base64-encoded PCM16 (16kHz mono) or float32 audio")
    ...
    initial_prompt: str | None = Field(default=None, description="Optional prompt context")
...
class TranscribeAndTranslateRequest(BaseModel):
    audio_base64: str = Field(..., description="Base64-encoded PCM16 (16kHz mono) audio")
...
async def lifespan(app: FastAPI) -> AsyncGenerator[None, None]:
    ...
    yield
    logger.info("[SeamlessApp] Shutting down SeamlessTranslation & WhisperASR Service...")
...
async def transcribe_audio(payload: TranscribeRequest) -> TranscribeResponse:
    if not asr_engine:
        return TranscribeResponse(text="", words=[], confidence=1.0, language=payload.language)
...
@app.post("/api/v1/asr/transcribe-and-translate", response_model=TranscribeAndTranslateResponse)
async def transcribe_and_translate(payload: TranscribeAndTranslateRequest) -> TranscribeAndTranslateResponse:
    if not asr_engine:
        return TranscribeAndTranslateResponse(
            text="", translation="", words=[], confidence=1.0, src_lang=payload.src_lang, tgt_lang=payload.tgt_lang
        )
